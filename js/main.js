...
const eventDate = new Date('2026-09-26T22:00:00-03:00').getTime();
...
document.querySelectorAll('main section').forEach((section, sectionIndex) => {
  ...
  const visual = section.querySelector('.countdown, .quote-card, .gallery-showcase, .location-card, .song-card, .dress-card, .gift-card, .rsvp-text');
...
copyCbuButton?.addEventListener('click', async () => {
  ...
  window.setTimeout(() => {
    ...
    copyCbuButton.textContent = 'Copiar alias';
  }, 2200);
});
...
songRequestForm?.addEventListener('submit', (event) => {
  ...
  if (!songName) {
    if (songStatus) songStatus.textContent = 'Escribí el nombre de una canción para continuar.';
    document.getElementById('songName')?.focus();
    return;
  }

  const artistText = songArtist ? ` de ${songArtist}` : '';
  const message = `Hola, quiero recomendar esta canción para los 15 de Antonella Pérez: “${songName}”${artistText}.`;
  const whatsappUrl = `https://wa.me/5493815636928?text=${encodeURIComponent(message)}`;

  if (songStatus) songStatus.textContent = 'Abriendo WhatsApp…';
